...
from chl.multiscale_local_patch.local_patch_DGCNN import MultiScaleLocalPatchesTorch
from chl.multiscale_local_patch.mscale_correlation import MultiScaleCorrelation
from chl.multiscale_local_patch.eliminate_outlier import MultiScaleLocalCon
...
class GeoTransformer(nn.Module):
    def __init__(self, cfg):
        ...
        self.multi_scale_local_patches_torch = MultiScaleLocalPatchesTorch(
            cfg.multi_scale_corr.scale_index, cfg.multi_scale_corr.scale
        )

        self.multi_scale_cor = MultiScaleCorrelation(cfg.multi_scale_corr.scale)

        self.coarse_matching = SuperPointMatching(
            cfg.coarse_matching.num_correspondences, cfg.coarse_matching.dual_normalization
        )

        self.eliminate_outlier = MultiScaleLocalCon(cfg.multi_scale_corr.co_scale)
    ...
    def forward(self, data_dict):
        ...
        # 4. Multi-scale Local Patches
        # 求他们的每个点的多尺度的局部的点特征
        # ref_multi_scale_local_patches = multi_scale_local_patches(ref_feats_c_norm,
        #                                                           ref_points_c,
        #                                                           ref_node_masks)
        #
        # src_multi_scale_local_patches = multi_scale_local_patches(src_feats_c_norm,
        #                                                           src_points_c,
        #                                                           src_node_masks)


        time1 = time.time()
        ref_multi_scale_local_patches = self.multi_scale_local_patches_torch(ref_feats_c_norm,
                                                                             ref_points_c,
                                                                             ref_node_masks)

        src_multi_scale_local_patches = self.multi_scale_local_patches_torch(src_feats_c_norm,
                                                                             src_points_c,
                                                                             src_node_masks)

        # 保存并输出对应的多尺度局部特征
        output_dict['ref_mscale_local_patches'] = ref_multi_scale_local_patches
        output_dict['src_mscale_local_patches'] = src_multi_scale_local_patches

        # 5. Calculate the correlation matrix of multi-scale Local Patches
        # 计算相关性矩阵
        # corr_matrix = multi_scale_correlation(multi_scale,
        #                                       ref_multi_scale_local_patches,
        #                                       src_multi_scale_local_patches,
        #                                       )
        with torch.no_grad():
            corr_matrix = self.multi_scale_cor(ref_multi_scale_local_patches, src_multi_scale_local_patches,)
        ...
        with torch.no_grad():
            ref_node_corr_indices, src_node_corr_indices, node_corr_scores = self.coarse_matching(
                ref_feats_c_norm, src_feats_c_norm, corr_matrix, gt_node_corr_indices, ref_node_masks, src_node_masks
            )

            time2 = time.time()
    
            ref_node_corr_indices, src_node_corr_indices, node_corr_scores = self.eliminate_outlier(
            ...
            # output_dict['mms_time'] = (time2 - time1) * 1000
            # output_dict['rms_time'] = (time3 - time2) * 1000
            # output_dict['tms_time'] = (time3 - time1) * 1000

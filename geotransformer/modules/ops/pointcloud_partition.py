...
def point_to_node_partition(
    ...
    min_limit = min(point_limit, sq_dist_mat.shape[1])

    node_knn_indices = sq_dist_mat.topk(k=min_limit, dim=1, largest=False)[1]  # (M, K)
    node_knn_node_indices = index_select(point_to_node, node_knn_indices, dim=0)  # (M, K)
    node_indices = torch.arange(nodes.shape[0]).cuda().unsqueeze(1).expand(-1, min_limit)  # (M, K)
